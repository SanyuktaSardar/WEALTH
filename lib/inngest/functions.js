...
// 1. Process individual recurring transaction (triggered by event)
...
// 3. Generate monthly reports on last day of each month at 23:00
  ...
  async ({ step }) => {
    ...
    for (const user of users) {
      await step.run(`generate-report-${user.id}`, async () => {
        ...
        // Fetch per-account budget data for this user
      ...
      await step.run(`check-budget-${budget.id}`, async () => {
        ...
        if (
          ...
          await sendEmail({
            ...
            subject: `Budget Alert: ${percentageUsed.toFixed(1)}% used for ${defaultAccount.name}`,
            react: EmailTemplate({
